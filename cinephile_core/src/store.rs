...
impl Store {
    ...
    pub async fn find_by_title(&self, title: &str) -> Result<Option<Movie>> {
        let mut results = self.db.query("SELECT * FROM movie WHERE title CONTAINS $title LIMIT 1")
            .bind(("title", title))
            .await?;
    ...
    pub async fn hybrid_search(&self, vector: Option<Vec<f32>>, filter_str: Option<String>) -> Result<Vec<Movie>> {
        ...
        if let Some(v) = vector {
            let mut scored_movies: Vec<(f32, Movie)> = movies.into_iter()
                .map(|m| (cosine_similarity(&v, &m.vector), m))
                .collect();
            ...
            Ok(movies.into_iter().take(10).collect())
        }
    }
}
